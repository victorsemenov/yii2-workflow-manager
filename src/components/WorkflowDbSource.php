<?php
namespace victorsemenov\workflow\manager\components;
...
class WorkflowDbSource extends BaseObject implements IWorkflowSource
{
    ...
    public function getStatus($id, $model = null)
    {
        ...
        if (!array_key_exists($canonicalStId, $this->_s[$wId])) {
            $statusModel = \victorsemenov\workflow\manager\models\Status::findOne([
                'workflow_id' => $wId,
                'id' => $stId
            ]);
            if ($statusModel == null) {
    ...
    public function getAllStatuses($workflowId)
    {
        if (empty($this->_allStatusLoaded[$workflowId])) {
            $this->_s[$workflowId] = [];
            /** @var \victorsemenov\workflow\manager\models\Status[] $statusModels */
            $statusModels = \victorsemenov\workflow\manager\models\Status::find()
    ...
    public function getTransitions($statusId, $model = null)
    {
        ...
        if (!array_key_exists($statusId, $this->_t[$wId])) {
            $transitions = [];
            $transitionModels = \victorsemenov\workflow\manager\models\Transition::find()
                ->andWhere([
     ...
     *
     * @param string $id
     * @return \victorsemenov\workflow\manager\models\Workflow
     * @throws WorkflowException
     * @internal param IWorkflowSource $source
     */
    public function loadDefinition($id)
    {
        $workflowModel = \victorsemenov\workflow\manager\models\Workflow::findOne(['id' => $id]);
        if (!$workflowModel) {
            return null;
